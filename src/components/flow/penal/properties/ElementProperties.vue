<template>
    <div class="panel-tab__content">
        <el-table :data="elementPropertyList"
                  size="mini"
                  max-height="240"
                  border
                  fit>
        ...
        <div class="element-drawer__button">
            <el-button size="mini"
                       type="primary"
                       icon="el-icon-plus"
                       @click="openAttributesForm(null, -1)">添加属性</el-button>
        </div>
...
export default {
    ...
    methods: {
        resetAttributesList() {
            ...
            if (this.bpmnElement && this.bpmnElement.businessObject && this.bpmnElement.businessObject.extensionElements && this.bpmnElement.businessObject.extensionElements.values) {
                this.bpmnElement.businessObject.extensionElements.values.forEach(e => {
                    if (e.$type !== `${this.prefix}:Properties`) {
                        this.otherExtensionList.push(e)
                    } else {
                        this.bpmnElementProperties.push(e)
                    }
                })
            }
            // 保存所有的 扩展属性字段
